...
set _OBS_TAG=26.1.0-rc1
set _Deps_VERSION=2017
...
::sed --version >nul 2>nul || powershell.exe -file "%OBS_ROOT%\scripts\install-build-reqs.ps1" || echo -- Error running scripts/install-build-reqs.ps1
...
:set_deps_hash
  set Deps_SHA1=d253eb16cf71aa65acda331fa703b7b8b9cf691f
  ...
  set OPENSSL_SHA1=a9d8556d2e0ece68cabe1fcde818e03b3745038e
  goto :eof
      ...
      if exist "%VS9E%" (set "VS=%VS9E%" & set "VS_VER=16 2019") else (
        if exist "%VS9P%" (set "VS=%VS9P%" & set "VS_VER=16 2019") else (
          if exist "%VS9C%" (set "VS=%VS9C%" & set "VS_VER=16 2019") else (
            ...
            powershell.exe -file "%OBS_ROOT%\scripts\install-winsdk.ps1"
            ...
            powershell.exe -file "%OBS_ROOT%\scripts\install-vs2019.ps1"
            if errorlevel 1 (
...
:download_dependencies
  ...
  @REM if not exist "%BOOST_INCLUDEDIR%\version.hpp" (
  @REM   echo -- Boost: downloading & curl -ksLRO --url "%BOOST_URL%"
  @REM   if errorlevel 1 (echo -- Boost: downloading - failed & exit /b)
  @REM   echo -- Boost: downloading - done
  @REM ) else (echo -- Boost: already installed: %BOOST_VERSION%)
...
:delete_downloads
  ...
  @REM if exist "%BOOST_FILENAME%" (del "%BOOST_FILENAME%")
